...
MODEL = "gpt-4o"   # ← fixed per request
...
def extract_text_from_pdf(file_like: io.BytesIO) -> str:
    """
    Convert PDF pages to images, OCR each, concatenate.
    """
    try:
        file_like.seek(0)
        ...
        for i, img in enumerate(images, 1):
            ...
            if len(ocr_text.strip()) < 30:
                logger.warning(f"OCR page {i} text too short; possible noise.")
                continue
            text_output += f"\n[Page {i}]\n{ocr_text}"
        return text_output
...
def extract_vehicle_from_text(text: str) -> Optional[str]:
    # year + make + model (loose) and odometer (if present)
...
def _image_is_exterior_wide(img: Image.Image) -> bool:
    """
    Heuristic: low OCR text + high variance → likely exterior wide shot (corner view).
    """
    processed = preprocess_image(img)
...
def check_required_photos(image_blobs: List[Tuple[str, bytes]], ocr_text: str) -> List[str]:
    ...
    - four corners satisfied if >= 2 exterior corner-like wide shots are present.
    - VIN/odo/plate can be detected via OCR signals in photos or hints in estimate text.
    """
    required = ["four corners", "odometer", "vin", "license plate"]
    present = set()
    txt = ocr_text.lower()
    ...
    if exterior >= 2:  # treat as satisfied
        present.add("four corners")

    missing = [p for p in required if p not in present]
    logger.debug(f"Photo check → present={sorted(list(present))}, missing={missing}, exterior_count={exterior}")
    return missing
...
def check_labor_and_tax_score(text: str, client_rules: str) -> int:
    """
    -50% if ALL labor sections are missing.
    -25% if rules require tax and estimate text has no tax signal.
    """
    adj = 0
    ...
    if not any(has_rate(lbl) for lbl in labels):
        ...
        logger.debug("At least one labor rate present → no labor deduction")
...
def compare_estimate_with_photos(items: List[Dict[str, str]],
    ...
    system = (
        ...
        "Hidden ops (calibration, internal R&I) may not be visible → mark as no-evidence and add a short note. "
        "Also list obvious damages seen in photos that are NOT listed in the estimate. "
        "Return STRICT JSON ONLY per this schema:\n" + json.dumps(schema) +
        "\nconfidence ∈ [0.0,1.0]; notes are 3–10 words."
    )
...
""".strip()
    ...
    if final_score < 100 and (labor_tax_adj == 0 and photo_adj == 0):
        # if the LLM deducted but our checks show no reason, normalize to 100
        final_score = 100

    # =========================================
    # PDF build (Helvetica core font → no TTF warnings)
    # =========================================
    pdf = FPDF()
    pdf.add_page()
    ...
    if consistency.get("per_item"):
        for it in consistency["per_item"][:40]:  # keep compact
    ...
    # OPTIONAL email (disabled by default—uncomment & set creds if needed)
    # try:
    #     msg = EmailMessage()
    ...
    #     # with smtplib.SMTP_SSL("mail.yourhost.com", 465) as smtp:
