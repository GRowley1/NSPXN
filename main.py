...
log = logging.getLogger("ultralite-gpt")
...
# -----------------------
# Client rules loader — EXACT docx by name (tiny tolerance only)
# -----------------------
...
async def get_client_rules(client_name: str):
    """
    Restore previous behavior: load the *exact* .docx for the selected carrier
    from CLIENT_RULES_DIR with only minimal tolerance for case and missing '.docx'.
    No fuzzy matching, no directory recursion, no extra changes.
    """
    base = client_name.strip()
    # Accept either raw name or already-suffixed name, case-insensitive
    candidates = []
    ...
    if not path:
        # As an absolute last resort (still non-fuzzy), try case-insensitive basename match in the top-level dir
        try:
    ...
    if not path or not os.path.exists(path):
        # Return the simple error text the UI expects
...
# -----------------------
# Main review: GPT does the work (with 1 guard: cannot be MATCH w/o estimate VIN)
# -----------------------
...
async def vision_review(
    ...
    # Minimal boolean: is there any estimate VIN in OCR text?
    has_estimate_vin = bool(re.search(r"\b[A-HJ-NPR-Z0-9]{17}\b", content_text))
    ...
    USER_TEXT = (
        ...
        f"There are {len(image_parts)} photos attached (order = Photo 1..{len(image_parts)}). Use them for VIN reading and evidence mapping.\n\n"
        "DETAIL LAYOUT for this request type:\n" + DETAIL_TEMPLATES.get(ai_intent, DETAIL_TEMPLATES["comprehensive"]) + "\n\n"
        "Constraints:\n"
        "- VIN must be a 17‑char uppercase string.\n"
        "- vin_verification must be MATCH or MISMATCH per the rules.\n"
        "- If no estimate VIN is found anywhere in the OCR text, set vin_verification=MISMATCH and clearly state 'No estimate VIN present' in the VIN section.\n"
        "- summary_brief: one paragraph (<=280 chars), plain text.\n"
        "- summary_markdown: full write‑up using the sections above."
    )
    ...
    # Minimal guard: if model said MATCH but we didn't find any estimate VIN in OCR text, force MISMATCH
    ...
    result = {
        ...
        "compliance_score": int(data.get("compliance_score") or 100),
